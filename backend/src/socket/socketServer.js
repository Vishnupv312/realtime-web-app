...
function createSocketServer(server) {
  ...
  function setupEventHandlers() {
    io.on("connection", (socket) => {
      ...
      socket.on("user:match:cancel", (data) =>
        handlers.handleCancelMatch(socket, data)
      );

      // Chat events
      socket.on("chat:message", (data) =>
        handlers.handleChatMessage(socket, data)
      );
      socket.on("chat:clear", () => handlers.handleChatClear(socket));

      // WebRTC signaling events
      socket.on("webrtc:offer", (data) =>
        handlers.handleWebRTCOffer(socket, data)
      );
      socket.on("webrtc:answer", (data) =>
        handlers.handleWebRTCAnswer(socket, data)
      );
      socket.on("webrtc:ice-candidate", (data) =>
        handlers.handleICECandidate(socket, data)
      );
      socket.on("webrtc:call-end", (data) =>
        handlers.handleWebRTCCallEnd(socket, data)
      );
      socket.on("webrtc:call-reject", (data) =>
        handlers.handleWebRTCCallReject(socket, data)
      );
      socket.on("webrtc:call-timeout", (data) =>
        handlers.handleWebRTCCallTimeout(socket, data)
      );
  ...
  async function handleTypingStart(socket) {
    try {
      ...
      const connectedUserSocketId = userSockets?.get(
        guestSession.connectedUser
      );
